...
namespace Penna {
  ...
  public:
    ...
    Population(T const & t , randgen_type & rng, const size_type nmax = 10, const size_type n = 0)
  : nmax_(nmax)
  // , population_(container_type())
  , distr_(t)
    ...
    void set_nmax(size_type n) { //why would you need to set nmax after
      //initialisation?
      nmax_ = n;
    }
  ...
  private:
	...
	if((it -> is_dead()) or random_death(nmax_, nbefore)(*it)) {
	  ...
	  //this will remove elements from population_!!
	}
	else {
	  ++it;
	}
